...
/**
 * JetBrains Protocol Command for handling Schmidt AI Coder authentication URLs
 *
...
class KiloCodeAuthProtocolCommand : JBProtocolCommand("ai.kilocode.jetbrains.auth") {
    ...
    override suspend fun execute(target: String?, parameters: Map<String, String>, fragment: String?): String? {
        logger.info("Handling Schmidt AI Coder auth protocol command: target=$target, parameters=$parameters")
        ...
        return try {
            ...
            val errorMsg = "Error handling Schmidt AI Coder auth protocol command: ${e.message}"
    ...
    private fun forwardTokenToVSCodeExtension(token: String) {
        coroutineScope.launch {
            try {
                ...
                val vscodeUriString = "vscode://kilocode.schmidt-ai-coder/schmidtaicoder?token=$token"
                ...
                extHostCommands.executeContributedCommand(
                    "schmidt-ai-coder.handleExternalUri",
