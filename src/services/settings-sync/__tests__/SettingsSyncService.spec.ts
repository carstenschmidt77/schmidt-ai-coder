	...
	describe("initialize", () => {
		it("should register sync keys when settings sync is enabled", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith([
				"schmidt-ai-coder.allowedCommands",
				"schmidt-ai-coder.deniedCommands",
				"schmidt-ai-coder.autoApprovalEnabled",
				"schmidt-ai-coder.fuzzyMatchThreshold",
				"schmidt-ai-coder.diffEnabled",
				"schmidt-ai-coder.directoryContextAddedContext",
				"schmidt-ai-coder.language",
				"schmidt-ai-coder.customModes",
				"schmidt-ai-coder.firstInstallCompleted",
				"schmidt-ai-coder.telemetrySetting",
			])
		...
		it("should use default value true when setting is not configured", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith(
				expect.arrayContaining(["schmidt-ai-coder.allowedCommands", "schmidt-ai-coder.deniedCommands"]),
			)
		})
		...
		it("should work without outputChannel parameter", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith(
				expect.arrayContaining(["schmidt-ai-coder.allowedCommands"]),
			)
		})
	})
