...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .schmidtaicodeignore exists
		 */
		it("should load .schmidtaicodeignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .schmidtaicodeignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".schmidtaicodeignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".schmidtaicodeignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .schmidtaicodeignore doesn't exist
		 */
		it("should allow all access when .schmidtaicodeignore doesn't exist", async () => {
			// Setup mocks to simulate missing .schmidtaicodeignore file
		 ...
		 */
		it("should set up file watcher for .schmidtaicodeignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".schmidtaicodeignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .schmidtaicodeignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .schmidtaicodeignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		beforeEach(async () => {
			// Setup .schmidtaicodeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**\n*.log")
			await controller.initialize()
		})
		...
		/**
		 * Tests the default behavior when no .schmidtaicodeignore exists
		 */
		it("should allow all access when no .schmidtaicodeignore content", async () => {
			// Create a new controller with no .schmidtaicodeignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .schmidtaicodeignore exists
		 */
		it("should allow all commands when no .schmidtaicodeignore exists", async () => {
			// Create a new controller with no .schmidtaicodeignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .schmidtaicodeignore
		 */
		it("should generate formatted instructions when .schmidtaicodeignore exists", async () => {
			// Setup .schmidtaicodeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
			...
			expect(instructions).toContain("# .schmidtaicodeignore")
		 ...
		 * Tests behavior when no .schmidtaicodeignore exists
		 */
		it("should return undefined when no .schmidtaicodeignore exists", async () => {
			// Setup no .schmidtaicodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .schmidtaicodeignore is created
		 */
		it("should reload .schmidtaicodeignore when file is created", async () => {
			// Setup initial state without .schmidtaicodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .schmidtaicodeignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .schmidtaicodeignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .schmidtaicodeignore is changed
		 */
		it("should reload .schmidtaicodeignore when file is changed", async () => {
			// Setup initial state with .schmidtaicodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .schmidtaicodeignore is deleted
		 */
		it("should reset when .schmidtaicodeignore is deleted", async () => {
			// Setup initial state with .schmidtaicodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
