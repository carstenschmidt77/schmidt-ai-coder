...
describe("CustomModesManager", () => {
	...
	describe("getCustomModes", () => {
		...
		it("should merge modes with .schmidtaicodermodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .schmidtaicodermodes since it takes precedence
		...
		it("should handle missing .schmidtaicodermodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .schmidtaicodermodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .schmidtaicodermodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .schmidtaicodermodes precedence", async () => {
			...
			// Should update global state with merged modes where .schmidtaicodermodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .schmidtaicodermodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .schmidtaicodermodes file when adding project-specific mode", async () => {
			...
			// Mock .schmidtaicodermodes to not exist initially
			...
			// Verify .schmidtaicodermodes was created with the project mode
			...
			// Verify the content written to .schmidtaicodermodes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".schmidtaicodermodes"),
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".schmidtaicodermodes"))
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should work with global custom modes when .roomodes doesn't exist", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.schmidtaicoder/rules-test-mode" },
			])
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a built-in mode customized in .roomodes", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.schmidtaicoder/rules-code" },
			])
